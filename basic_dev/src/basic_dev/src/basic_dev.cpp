...
{  
    ...
    velcmd.yawRate = 0; // z方向角速度(yaw, rad/s)
    velcmd.vx = 0;      // x方向线速度(m/s)
    velcmd.vy = 0;      // y方向线速度(m/s)
    velcmd.vz = 0;      // z方向线速度(m/s)
    velcmd.va = 4;      // 加速度档位(0~8)
    velcmd.stop = 0;    // 0:正常飞行 1:急停
    ...
    vel_publisher = nh->advertise<airsim_ros::VelCmd>("/airsim_node/drone_1/vel_cmd_body_frame", 10);
    // Compatibility channel used by some bridge versions.
    vel_body_publisher = nh->advertise<airsim_ros::VelCmd>("/airsim_node/drone_1/vel_body_cmd", 10);

    ros::AsyncSpinner spinner(2);
    spinner.start();

    ros::Rate wait_rate(20);
    ROS_INFO("Waiting for pose_gt...");
    ...
    ROS_INFO("Forward mission: fly forward continuously at %.1f m/s", forward_speed);

    ros::Rate loop_rate(30);
    while (ros::ok()) {
        velcmd.header.stamp = ros::Time::now();
        velcmd.vx = forward_speed;
        velcmd.vy = 0.0f;
        velcmd.vz = 0.0f;
        velcmd.yawRate = 0.0f;
        velcmd.stop = 0;
        vel_publisher.publish(velcmd);
        vel_body_publisher.publish(velcmd);
        ROS_INFO_THROTTLE(1.0, "Publishing VelCmd: vx=%.2f vy=%.2f vz=%.2f yawRate=%.2f stop=%u",
                          velcmd.vx, velcmd.vy, velcmd.vz, velcmd.yawRate, velcmd.stop);
        loop_rate.sleep();
    }
